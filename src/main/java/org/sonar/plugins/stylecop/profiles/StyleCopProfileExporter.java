...
public class StyleCopProfileExporter extends ProfileExporter {
  ...
  private void printRules(RulesProfile profile, Writer writer) throws IOException {
    List<ActiveRule> activeRules = profile.getActiveRulesByRepository(StyleCopPlugin.REPOSITORY_KEY);
    List<org.sonar.plugins.stylecop.profiles.StyleCopRule> rules = transformIntoStyleCopRules(activeRules);
